...
export class PDFProcessingService {
   ...
   */
  static async extractTextFromPDF(filePath: string): Promise<ExtractedPDFData> {
    try {
   ...
   */
  static async processEditalWithAI(
    pdfText: string,
    contestInfo?: { institution?: string; position?: string; examDate?: string }
...
`;
   ...
   */
  private static validateAndFormatProcessedData(data: any): ProcessedEditalData {
   ...
   */
  static async calculatePriorityBasedOnHistory(
    topicName: string,
    institution: string
  ): Promise<number> {
   ...
   */
  static calculateCardsBasedOnExamDate(
    basecards: number,
    examDate?: string
  ): number {
    if (!examDate) return basecards;
    ...
    try {
      ...
      if (daysUntilExam <= 0) return basecards;
