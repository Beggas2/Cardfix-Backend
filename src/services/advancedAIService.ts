...
// Definindo um tipo para o Contest com as relações incluídas
type ContestWithRelations = Prisma.ContestGetPayload<{
  include: {
    contestTopics: {
      include: {
        topic: {
...
export class AdvancedAIService {
  ...
  async analyzeSubtopicPriorities(
    ...
    try {
      ...
      const historicalContests: ContestWithRelations[] = await prisma.contest.findMany({
        where: {
          OR: [
  ...
  async generateIntelligentCards(params: GenerateCardsParams & {
    ...
    try {
      ...
      const priorityData = await this.analyzeSubtopicPriorities(
        aiParams.contestType || '',
        aiParams.institution || ''
      );
      
      const subtopicPriority = priorityData.find(
        p => p.subtopicName === aiParams.subtopicName && p.topicName === aiParams.topicName
      );
