...
import { generateCardsWithRetry } from './aiService';
...
export const parsePDFContent = async (filePath: string): Promise<string> => {
  try {
...
export const extractTopicsFromText = async (text: string, contestName: string): Promise<ParsedEditalData> => {
...
`;
  ...
  try {
    ...
    return {
      ...
      totalSubtopics: parsedData.topics?.reduce((acc: number, topic: any) => acc + (topic.subtopics?.length || 0), 0) || 0,
...
export const calculateTopicPriorities = async (
  contestName: string,
  ...
  const defaultPriorities: TopicPriority[] = [
...
export const generateCardsForSubtopic = async (
  subtopicId: string,
  topicName: string,
  subtopicName: string,
  contestName: string,
...
export const processEditalWithAI = async (
  contestId: string,
  filePath: string,
  contestName: string,
  selectedOffice: string,
  ...
  try {
    ...
    const extractedText = await parsePDFContent(filePath);

    // 2. Extrair tópicos e subtópicos usando IA
    console.log('Extraindo tópicos com IA...');
    const parsedData = await extractTopicsFromText(extractedText, contestName);

    // 3. Calcular prioridades
    console.log('Calculando prioridades...');
    const priorities = await calculateTopicPriorities(contestName, selectedOffice, examDate);
    ...
    for (const topicData of parsedData.topics) {
      ...
      for (const subtopicName of topicData.subtopics) {
        ...
        const priority = priorities.find(
          p => p.topicName === topicData.name && p.subtopicName === subtopicName
        );

        if (priority) {
          // Gerar cards para este subtópico
          await generateCardsForSubtopic(
            subtopic.id,
            topicData.name,
            subtopicName,
            contestName,
